...
{
    /// <summary>
    /// Interaction logic for BorderLoop.xaml
    /// </summary>
    public partial class BorderLoop : UserControl
    {
        private static readonly DependencyProperty _EdgeDuration = DependencyProperty.Register("EdgeDuration", typeof(int), typeof(BorderLoop), new PropertyMetadata(200, (DependencyObject d, DependencyPropertyChangedEventArgs eventArgs) => { ((BorderLoop)d).EdgeDuration = (int)eventArgs.NewValue; }));
        private static readonly DependencyProperty _FadeOutDuration = DependencyProperty.Register("FadeOutDuration", typeof(int), typeof(BorderLoop), new PropertyMetadata(200, (DependencyObject d, DependencyPropertyChangedEventArgs eventArgs) => { ((BorderLoop)d).FadeOutDuration = (int)eventArgs.NewValue; }));
        private static readonly DependencyProperty _FadeOutDelay = DependencyProperty.Register("FadeOutDelay", typeof(int), typeof(BorderLoop), new PropertyMetadata(300, (DependencyObject d, DependencyPropertyChangedEventArgs eventArgs) => { ((BorderLoop)d).FadeOutDelay = (int)eventArgs.NewValue; }));
        private static readonly DependencyProperty _BColor = DependencyProperty.Register("BColor", typeof(Brush), typeof(BorderLoop), new PropertyMetadata(new SolidColorBrush(Color.FromRgb(174, 234, 0)), (DependencyObject d, DependencyPropertyChangedEventArgs eventArgs) => { ((BorderLoop)d).BColor = (Brush)eventArgs.NewValue; }));
        ...
        private void InitSbs()
        {
            ...
            rightEdgeSb = CreateSb("RightEdgeCol1", "RightEdgeCol2", "RightEdge", false, EdgeDuration);
            bottomEdgeSb = CreateSb("BottomEdgeCol1", "BottomEdgeCol2", "BottomEdge", true, EdgeDuration * 2);
            leftEdgeSb = CreateSb("LeftEdgeCol1", "LeftEdgeCol2", "LeftEdge", false, EdgeDuration * 3);
        }
        ...
        private Storyboard CreateSb(string colNameOne, string colNameTwo, string barName, bool isCol, int beginTime)
        {
            ...
            DoubleAnimation fadeOutAnim = CreateFadeOutAnim(barName, (EdgeDuration * 4) + FadeOutDelay, FadeOutDuration, false);

            int resetTime = ((EdgeDuration * 4) + FadeOutDelay) + FadeOutDuration;
